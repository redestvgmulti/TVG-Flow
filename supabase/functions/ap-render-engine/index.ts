...
const NO_BLOCKED_LAYERS = new Set<string>();
function addImage(layers: Record<string, any>, layer: string | undefined, url: string | null, blockedLayers: Set<string> = NO_BLOCKED_LAYERS) { if (layer && url && !blockedLayers.has(layer)) layers[layer] = { image: url }; }
function addText(layers: Record<string, any>, layer: string | undefined, text: string | null | undefined, blockedLayers: Set<string> = NO_BLOCKED_LAYERS) { if (layer && text && !blockedLayers.has(layer)) layers[layer] = { text }; }
...
Deno.serve(async (req) => {
  ...
  for (const item of items) {
    ...
    try {
      ...
      const renderFormat = item.content_type || "feed";
      const background = renderFormat === "feed" ? (item.imagem_url || (item.imagem_storage ? `${Deno.env.get("SUPABASE_URL")}/storage/v1/object/public/ap-images/${item.imagem_storage}` : null)) : null;
      ...
      if (master.enabled) {
        ...
        const blockedLayers = renderFormat === "reels" ? REELS_FIXED_LAYERS : NO_BLOCKED_LAYERS;
        addText(nextLayers, map.headline, item.headline, blockedLayers);
        if (renderFormat === "feed") addText(nextLayers, map.tag, item.context_tag || "DESTAQUE", blockedLayers);
        if (renderFormat === "feed") addImage(nextLayers, map.news_image, background, blockedLayers);
        addImage(nextLayers, map.visual_title, assetUrl(Deno.env.get("SUPABASE_URL")!, snapshot.visual_title), blockedLayers);
        for (const [slot, asset] of Object.entries(snapshot.sponsor_profile?.slots || {})) addImage(nextLayers, map[slot], assetUrl(Deno.env.get("SUPABASE_URL")!, asset), blockedLayers);
