...
export function isVisualModelAllowedForFormat(
  ...
  return (VISUAL_MODEL_FORMATS[visualModel] as readonly string[]).includes(
    contentType,
  );
}

export function sponsorCountFromConfig(
  config: Record<string, unknown>,
): number {
  const count =
    config.sponsor_count === null || config.sponsor_count === undefined
      ? Number.NaN
...
export function masterConfigIssues(
  ...
  if (!nonEmpty(config.master_template_uuid)) {
...
export async function requireMasterConfiguration(input: {
  ...
  if (
    masterConfigIssues(config, input.contentType, input.visualModel).length > 0
  ) {
    throw new MasterConfigurationError(
      "MASTER_CONFIG_INVALID",
      "master_render_configs",
