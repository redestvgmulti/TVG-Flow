...
import { detectImageType, fetchPublicBytes } from "../_shared/safeEgressFetcher.mjs";
...
async function uploadThumbnail(supabase: SupabaseClient, clienteId: string, sourceId: string, item: InstagramItem): Promise<string | null> {
    ...
    try {
        const image = await fetchPublicBytes(item.thumbnailUrl, {
            maxBytes: 10 * 1024 * 1024,
            timeoutMs: 10000,
            allowedContentTypes: ['image/jpeg', 'image/png', 'image/webp'],
        });
        const detectedType = detectImageType(image.bytes);
        if (!detectedType || detectedType.contentType !== image.contentType) return null;

        const imageId = await sha256(item.externalId);
        const path = `ingestion/instagram/${clienteId}/${sourceId}/${imageId}.${detectedType.extension}`;
        const { error } = await supabase.storage.from("ap-images").upload(path, image.bytes, {
...
Deno.serve(async (req: Request) => {
  ...
  let sourceQuery = supabase
    ...
    .select("id, cliente_id, nome, url, tipo, last_success_at, consecutive_failures")
    .eq("tipo", "instagram")
    .eq("ativo", true);
  ...
  try {
      ...
      return json({ error: "APIFY_PROVIDER_FAILED" }, 500);
  }
