// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// AutoPublisher — Image Fetcher Worker (With Telemetry)
// Refactored: 2026-03-25 — Reels Integration & Image-Free Flow.
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

import "jsr:@supabase/functions-js/edge-runtime.d.ts";
import { createClient } from "npm:@supabase/supabase-js@2";
import { Telemetry } from "../_shared/telemetry.ts";

const BATCH_LIMIT = 10;
const BUCKET = "ap-images";
const LOCK_EXPIRY_MINUTES = 10;

Deno.serve(async (_req: Request) => {
    const supabase = createClient(
        Deno.env.get("SUPABASE_URL")!,
        Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!,
        { auth: { autoRefreshToken: false, persistSession: false } }
    );

    const workerId = crypto.randomUUID();
    const expiryCutoff = new Date(Date.now() - LOCK_EXPIRY_MINUTES * 60 * 1000).toISOString();

    const { data: items } = await supabase
        .schema("ap").from("candidate_news")
        .select("id, imagem_url, url_original, cliente_id, content_type")
        .eq("status", "raw")
        .or(`processing_started_at.is.null,processing_started_at.lt.${expiryCutoff}`)
        .order("created_at", { ascending: true })
        .limit(BATCH_LIMIT);

    const results = [];

    for (const item of items ?? []) {
        const telemetry = new Telemetry(supabase);
        await telemetry.logStart({ worker_name: "ap-image-fetcher", worker_id: workerId, news_id: item.id, cliente_id: item.cliente_id });

        const lockTime = new Date().toISOString();
        const { data: locked } = await supabase
            .schema("ap").from("candidate_news")
            .update({ processing_started_at: lockTime })
            .eq("id", item.id)
            .eq("status", "raw")
            .or(`processing_started_at.is.null,processing_started_at.lt.${expiryCutoff}`)
            .select("id");

        if (!locked?.length) {
            await telemetry.logError("acquire_failed");
            continue;
        }

        try {
            let storagePath = null;
            let targetImgUrl = item.imagem_url;

            // ⚠️ REELS AUDIT: Skip image fetch for reels
            if (item.content_type !== "reels" && targetImgUrl) {
                const imgRes = await fetch(targetImgUrl).catch(() => null);
                if (imgRes?.ok) {
                    const contentType = imgRes.headers.get("content-type") || "image/jpeg";
                    const buffer = new Uint8Array(await imgRes.arrayBuffer());
                    storagePath = `${item.id}.${contentType.includes("png") ? "png" : "jpg"}`;
                    await supabase.storage.from(BUCKET).upload(storagePath, buffer, { contentType, upsert: true });
                }
            } else if (item.content_type === "reels") {
                console.log(`[Reels-Audit] Skipping image fetch for item ${item.id} (Reels Flow)`);
            }

            await supabase
                .schema("ap").from("candidate_news")
                .update({ 
                    imagem_storage: storagePath, 
                    status: "ready_for_scoring", 
                    processing_started_at: null, 
                    updated_at: new Date().toISOString() 
                })
                .eq("id", item.id)
                .eq("processing_started_at", lockTime);

            await telemetry.logSuccess();
            results.push({ id: item.id, status: "success" });

        } catch (err: any) {
            await supabase.schema("ap").from("candidate_news").update({ processing_started_at: null }).eq("id", item.id).eq("processing_started_at", lockTime);
            await telemetry.logError(err.message);
        }
    }

    return new Response(JSON.stringify({ ok: true, processed: results.length }));
});
