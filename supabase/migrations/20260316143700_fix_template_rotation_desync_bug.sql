-- Fix for ap.get_and_advance_template infinity loop bug
-- 
-- The previous version used:
--     UPDATE ap.template_queue_state 
--     SET current_index = current_index + 1
--
-- This caused an infinite loop if the DB column `current_index` exceeded `v_total_templates`.
-- The local variable `v_current_index` was reset to 1, but the `UPDATE` used the column name
-- `current_index`, causing the DB value to increment indefinitely (e.g., 100 -> 101) 
-- while the function always returned the first template.
-- This migration fixes the UPDATE to use `v_current_index + 1` and adds a NULL safeguard.
...
BEGIN
    ...
    -- 3.5 Fallback se current_index desincronizar (Bug Fix)
    ...
    IF v_selected_template IS NULL THEN
        -- Fallback de emergência caso OFFSET falhe
        SELECT id, placid_template_uuid, ordem, nome 
        INTO v_selected_template
        FROM ap.templates
        WHERE empresa_id = p_empresa_id AND ativo = true AND tipo = p_tipo AND template_set = v_effective_set
    ...
    IF v_current_index >= v_total_templates THEN
        ...
        WHERE empresa_id = p_empresa_id AND tipo = p_tipo AND template_set = v_effective_set;
    ELSE
        -- BUG FIX: Usar v_current_index + 1 em vez de referenciar a coluna que pode estar desincronizada
        UPDATE ap.template_queue_state 
