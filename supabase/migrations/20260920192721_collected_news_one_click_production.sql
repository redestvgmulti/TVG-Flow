-- One-click collected-news production.
--
-- The entry RPC composes the existing collection approval, backlog adoption,
-- and canonical article-start RPCs in one transaction. It does not create a
-- candidate, render, approve, or publish anything.

BEGIN;

ALTER TABLE ap.editorial_article_sources
    ADD COLUMN IF NOT EXISTS source_collected_news_id uuid
        REFERENCES ap.collected_news(id) ON DELETE RESTRICT,
...
ALTER TABLE ap.editorial_article_sources
    DROP CONSTRAINT editorial_article_sources_url_shape_check;
ALTER TABLE ap.editorial_article_sources
    ADD CONSTRAINT editorial_article_sources_url_shape_check CHECK (
        (source_type IN ('link', 'news_backlog') AND source_url ~ '^https?://')
        OR (source_type NOT IN ('link', 'news_backlog') AND source_url IS NULL)
    );

CREATE OR REPLACE FUNCTION ap.start_collected_news_editorial_production(
...
DECLARE
    ...
    v_existing_article ap.editorial_articles%ROWTYPE;
    v_approval jsonb;
    v_source_name text;
    v_source_captured boolean;
    v_source_sufficient boolean;
BEGIN
    IF v_user_id IS NULL THEN RAISE EXCEPTION 'AUTH_REQUIRED' USING ERRCODE = '28000'; END IF;
    IF p_collected_news_id IS NULL OR p_request_id IS NULL THEN
    ...
    -- CollectedNewsPanel is an administrative inbox. Keep that existing
    -- authority boundary while deriving the tenant server-side.
    PERFORM 1 FROM ap.require_editorial_admin_access(v_cliente_id);

    SELECT collected.* INTO v_collected
    FROM ap.collected_news AS collected
    WHERE collected.id = p_collected_news_id
      AND collected.cliente_id = v_cliente_id
    FOR UPDATE;
    ...
    IF v_collected.status = 'pending_review' THEN
        SELECT ap.approve_collected_news(v_cliente_id, v_collected.id, NULL)
          INTO v_approval;
        SELECT collected.* INTO v_collected
        FROM ap.collected_news AS collected
        WHERE collected.id = p_collected_news_id
        FOR UPDATE;
    ELSIF v_collected.status NOT IN ('approved', 'duplicate') THEN
        RAISE EXCEPTION 'COLLECTED_NEWS_NOT_PRODUCIBLE' USING ERRCODE = '55000';
    END IF;

    IF v_collected.news_backlog_id IS NOT NULL THEN
        SELECT * INTO v_backlog FROM ap.news_backlog
        WHERE id = v_collected.news_backlog_id AND cliente_id = v_cliente_id
        FOR UPDATE;
    ELSIF v_collected.status = 'duplicate' THEN
        SELECT * INTO v_backlog FROM ap.news_backlog
        WHERE id = NULLIF(v_collected.metadata->>'duplicate_backlog_id', '')::uuid
          AND cliente_id = v_cliente_id
        FOR UPDATE;
    END IF;
    IF NOT FOUND THEN RAISE EXCEPTION 'BACKLOG_NOT_FOUND' USING ERRCODE = 'P0002'; END IF;
    ...
    FOR UPDATE;
    IF FOUND AND v_existing_article.responsible_user_id IS DISTINCT FROM v_user_id THEN
        RAISE EXCEPTION 'COLLECTED_NEWS_ALREADY_IN_PRODUCTION' USING ERRCODE = '42501';
    END IF;

    IF v_backlog.status = 'available' AND v_backlog.adopted_by_user_id IS NULL THEN
        SELECT * INTO v_backlog
        FROM ap.adopt_news_backlog_item(v_backlog.id, v_cliente_id);
    ELSIF v_backlog.status IN ('adopted', 'in_production')
          AND v_backlog.adopted_by_user_id = v_user_id THEN
        NULL; -- idempotent retry/reopen by the owner
    ELSE
        RAISE EXCEPTION 'COLLECTED_NEWS_ALREADY_IN_PRODUCTION' USING ERRCODE = '42501';
    END IF;

    SELECT * INTO v_article
    FROM ap.start_editorial_article_from_backlog(v_backlog.id, p_request_id);

    SELECT source.nome INTO v_source_name
    FROM ap.sources AS source WHERE source.id = v_collected.source_id;
    SELECT EXISTS (
        SELECT 1 FROM ap.editorial_article_sources AS article_source
        WHERE article_source.article_id = v_article.id
    ) INTO v_source_captured;
    v_source_sufficient := length(btrim(COALESCE(v_collected.content, ''))) >= 200;

    RETURN jsonb_build_object(
        'article', to_jsonb(v_article),
        'backlog', to_jsonb(v_backlog),
        'collected_news', jsonb_build_object(
            'id', v_collected.id,
            'source_id', v_collected.source_id,
            'source_name', v_source_name,
            'url_original', v_collected.url_original,
            'canonical_url', v_collected.canonical_url,
            'title', v_collected.title,
            'excerpt', v_collected.excerpt,
            'content', v_collected.content,
            'image_url', v_collected.image_url,
            ...
            'metadata', v_collected.metadata
        ),
        'source_sufficient', v_source_sufficient,
        'source_requires_scrape', NOT v_source_sufficient,
        'source_captured', v_source_captured
    );
END;
$function$;
...
BEGIN
    ...
    IF v_source_sufficient THEN
        ...
        v_image := NULLIF(btrim(v_collected.image_url), '');
    ELSE
        v_title := COALESCE(NULLIF(btrim(p_scraped_title), ''), NULLIF(btrim(v_collected.title), ''));
        v_body := NULLIF(btrim(p_scraped_body), '');
        v_image := COALESCE(NULLIF(btrim(v_collected.image_url), ''), NULLIF(btrim(p_scraped_image_url), ''));
...
REVOKE ALL ON FUNCTION ap.start_collected_news_editorial_production(uuid, uuid) FROM PUBLIC, anon, service_role;
GRANT EXECUTE ON FUNCTION ap.start_collected_news_editorial_production(uuid, uuid) TO authenticated;
REVOKE ALL ON FUNCTION ap.capture_collected_news_article_source(uuid, uuid, text, text, text, uuid) FROM PUBLIC, anon, service_role;
GRANT EXECUTE ON FUNCTION ap.capture_collected_news_article_source(uuid, uuid, text, text, text, uuid) TO authenticated;

COMMENT ON FUNCTION ap.start_collected_news_editorial_production(uuid, uuid) IS
    'Atomically approves if needed, exclusively adopts, and starts/reuses one canonical article for a collected news item.';
COMMENT ON FUNCTION ap.capture_collected_news_article_source(uuid, uuid, text, text, text, uuid) IS
    'Captures one append-only news_backlog source, using collected content when sufficient or an ap-link-scraper result when required.';
