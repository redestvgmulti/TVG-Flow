-- Adds a discard action for the shared news backlog: prune an unclaimed link,
-- or a pauta the current user adopted but decided not to pursue. Distinct from
-- release (returns an adopted item to the pool for someone else to claim).

-- 'archived' previously required adopted_by/adopted_at to be set (it only
-- covered post-adoption archiving). Discarding a never-adopted 'available'
-- item now also lands on 'archived', so the consistency check must allow
-- archived rows with no adopter on record.
ALTER TABLE ap.news_backlog DROP CONSTRAINT news_backlog_adoption_consistency_check;
ALTER TABLE ap.news_backlog ADD CONSTRAINT news_backlog_adoption_consistency_check
    CHECK (
...
ALTER TABLE ap.news_backlog_events ADD CONSTRAINT news_backlog_event_action_check
    CHECK (action IN ('created', 'adopted', 'released', 'production_started', 'discarded'));
...
-- Archived items are permanently excluded from the shared board; stop sending
-- them to every client on every poll.
