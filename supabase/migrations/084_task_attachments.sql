...
CREATE TABLE IF NOT EXISTS task_attachments (
  ...
  CONSTRAINT task_attachments_filename_not_empty 
    CHECK (length(filename) > 0),

  CONSTRAINT task_attachments_original_filename_not_empty 
    CHECK (length(original_filename) > 0)
);

-- Indexes for performance
CREATE INDEX IF NOT EXISTS idx_task_attachments_tarefa 
  ON task_attachments(tarefa_id);

CREATE INDEX IF NOT EXISTS idx_task_attachments_empresa 
  ON task_attachments(empresa_id);
...
-- Comments
COMMENT ON TABLE task_attachments 
  IS 'Stores metadata for files attached to tasks';

COMMENT ON COLUMN task_attachments.filename 
  IS 'UUID-based filename stored in Supabase Storage';

COMMENT ON COLUMN task_attachments.original_filename 
  IS 'Original filename uploaded by user';

COMMENT ON COLUMN task_attachments.storage_path 
  IS 'Full path in storage bucket: {empresa_id}/{tarefa_id}/{uuid}_{filename}';

COMMENT ON COLUMN task_attachments.file_size 
  IS 'File size in bytes (max 10MB)';
