...
> **Atualização pós-auditoria (mesmo dia)**: os dois achados classificados
> abaixo como bloqueador (seção 5.1) e achado de segurança (seção 5.3) foram
> corrigidos por uma migration aditiva
> (`20260918100000_2b2_editorial_visibility_and_grants_hardening.sql`),
> commitada no topo deste mesmo branch, com 10 novos testes automatizados
> provando a correção. Ver seção 9 ("Resolução") para o registro completo.
> Os vereditos do gate (seção 8) já refletem o estado pós-correção.
...
## 2. Diff integrado (`origin/main...HEAD`)

> Contagem original desta auditoria: 68 arquivos, +8983/−143. Depois da
> correção registrada na seção 9, mais 3 arquivos: a migration
> `20260918100000_2b2_editorial_visibility_and_grants_hardening.sql`
> (MIGRATION), o teste `2b2-editorial-visibility-and-grants-hardening.test.mjs`
> (TEST) e a atualização de `tests/migrations/2b1-fixture.mjs` (TEST) para
> aplicá-la. Nenhuma categoria nova precisou ser criada.
...
| 10 | **Flag OFF depois de criar artigo — recovery path** | `[TESTE AUTOMATIZADO]` `2b2-editorial-visibility-and-grants-hardening.test.mjs` (commitado) | **RESOLVIDO — ver seção 9** |
...
**Achado real, confirmado empiricamente, RESOLVIDO** (ver seção 9): duas
RPCs `authenticated`-facing (`save_editorial_article_draft` e
...
deixava executáveis por `anon`/`PUBLIC` por padrão do Postgres (que concede
`EXECUTE` a `PUBLIC` em toda função nova, a menos que revogado
explicitamente). **Severidade real baixa**: `auth.uid()` resolve `NULL` para
`anon`, então a própria função já rejeitava com `AUTH_REQUIRED` antes de
qualquer leitura/escrita — não havia vazamento de dados nem escrita não
autorizada demonstrável. Mas era uma quebra real e mensurável do padrão de
defesa em profundidade que o resto do stack (P0 incluído) segue à risca.
Corrigido por migration aditiva (seção 9).
...
### 5.1 BLOQUEADOR (RESOLVIDO — ver seção 9) — flag OFF esconde artigos já existentes
...
Isso é exatamente o cenário que a seção 10 do pedido de auditoria original
definiu como critério de bloqueio: **"Se hoje não houver recovery path
suficiente: classificar como bloqueador de rollout."** Foi classificado
como tal, e corrigido (seção 9 deste relatório).
...
**Status: RESOLVIDO.** Ver seção 9 para a migration de correção, os testes
que a comprovam e a re-execução completa das suítes.
...
### 5.3 Grants ausentes (RESOLVIDO — ver seção 9) — comprovado empiricamente
...
análise de severidade e seção 9 para a correção.
...
14 migrations do stack 2B, em ordem cronológica real (a 14ª adicionada na
correção registrada na seção 9):
...
Rodado nesta sessão, contra Postgres efêmero real, **depois** da correção da
seção 9 (10 testes novos incluídos):
...
```text
READINESS DOCS COMMITTED:             PASS
FLAG OFF EXISTING ARTICLE VISIBILITY: PASS (corrigido e comprovado — seção 9)
NEW ARTICLE CREATION BLOCKED WHEN FLAG OFF: PASS
ADMIN VISIBILITY:                     PASS
STAFF VISIBILITY:                     PASS
CROSS-TENANT ISOLATION:               PASS
PUBLIC EXECUTE ON DRAFT RPC:          NO (corrigido — seção 9)
ANON EXECUTE ON DRAFT RPC:            NO (corrigido — seção 9)
PUBLIC EXECUTE ON FINALIZE RPC:       NO (corrigido — seção 9)
ANON EXECUTE ON FINALIZE RPC:         NO (corrigido — seção 9)
...
MY WORK:                              PASS
CENTRAL EDITORIAL:                    PARTIAL (por desenho, conforme pedido -- cobertura completa dos 3 estados que precisa cobrir; reorganização das abas legadas fica para depois)
LEGACY FALLBACK:                      PASS
2B TESTS:                             PASS
NEW REGRESSIONS:                      0 / 572
PRODUCTION MODIFIED:                  NO
READY TO MERGE 2B:                    YES
READY FOR CONTROLLED 2B DEPLOY:       YES
```

**READY TO MERGE 2B = YES**: o código integra limpo, testa limpo (553/561
executáveis passando, 0 regressão nova), não modifica P0, não expõe RPCs
service-role, e os dois achados reais que existiam nesta auditoria (seções
5.1 e 5.3) já foram corrigidos e comprovados por teste automatizado (seção 9).

**READY FOR CONTROLLED 2B DEPLOY = YES**: com a correção da seção 9 aplicada
e testada, não resta bloqueador conhecido para ligar a flag em um tenant
piloto. Permanecem como recomendável, não bloqueador: (a) executar os
checklists de QA manual no navegador antes do piloto (seção 7); (b) a
triagem separada do bug em `_shared/editorialAdminAuth.ts` (fora do escopo
de 2B, seção 2.1) antes do merge de PR #13, já que ele entra em `main`
junto por ancestralidade de branch.

---

## 9. Resolução dos achados críticos (implementada nesta sessão)

Ambos os achados da auditoria original (seções 5.1 e 5.3) foram corrigidos
nesta mesma sessão, no topo do stack (`feat/editorial-2b2-3-ui-wiring`), por
uma única migration aditiva:

`supabase/migrations/20260918100000_2b2_editorial_visibility_and_grants_hardening.sql`

**Não editou nenhuma migration já commitada** — nem a de
`2b1_editorial_state_machine_expansion` (que introduziu o early-return por
flag) nem a de `2b2_editorial_admin_tenant_visibility` (que reafirmou o
corpo mais recente antes desta correção). `list_my_editorial_articles`
mantém a mesma assinatura (`p_cliente_id uuid DEFAULT NULL`) em toda a sua
história, então `CREATE OR REPLACE` preserva os grants já corretos dela sem
precisar de um novo `GRANT`.

### 9.1 Correção 1 — visibilidade não depende mais da flag

O corpo de `ap.list_my_editorial_articles` perdeu o bloco
`IF NOT EXISTS (... flag ON ...) THEN RETURN; END IF;`. Toda a regra de
quem vê o quê permanece idêntica (admin vê todos do tenant, staff só os
próprios, `abandoned` excluído, `LEFT JOIN` preservando artigos de origem
direta) — a única mudança é que a checagem da flag não participa mais da
decisão de visibilidade, só da decisão de escrita (que já era feita, de
forma independente, por `ap.assert_editorial_workflow_v1_enabled` dentro de
cada RPC de mutação).

### 9.2 Correção 2 — grants explícitos para as RPCs de 5 argumentos

Adicionado, sem tocar nenhum corpo de função:
...
### 9.3 Testes novos — `tests/migrations/2b2-editorial-visibility-and-grants-hardening.test.mjs` (10 casos, todos passando)

Cenário obrigatório do pedido, provado ponta a ponta contra Postgres real:
flag ON → cria artigo → salva rascunho → lista (aparece para autor e admin)
→ flag OFF → lista de novo → **artigo continua aparecendo** para autor e
admin, `get_editorial_article_for_edit` continua devolvendo o conteúdo
intacto. Mais:

- staff sem vínculo com o artigo continua sem vê-lo, com a flag em
  qualquer estado;
- admin de outro tenant continua vendo zero linhas;
- criação de artigo novo continua bloqueada com a flag desligada
  (`EDITORIAL_WORKFLOW_DISABLED`) — a correção não afrouxou o lado de
  escrita;
- checagem de privilégio via `has_function_privilege` direto no
  `pg_catalog`: `anon=false`, `PUBLIC=false`, `service_role=false`,
  `authenticated=true` para as duas funções de 5 argumentos;
- **prova funcional, não só de catálogo**: uma chamada real como `anon`
  (`SET ROLE anon`) é rejeitada pelo próprio PostgreSQL com
  `permission denied for function ...` — antes mesmo de qualquer lógica
  interna da função rodar. Isso responde diretamente ao princípio do
  pedido: a proteção não depende só do `auth.uid()` interno;
- fluxo de negócio ponta a ponta (criar → rascunho → intenção de produção →
  finalizar) continua idêntico como `authenticated` — a correção de grants
  não alterou nenhuma regra de negócio.

### 9.4 Re-execução completa depois da correção

```
R1 + 2B.1 + 2B.2.1 + 2B.2.2 (migrations): 76 pass / 0 fail / 3 skip (pré-existentes)
P0 (tests/p0/*.test.mjs):                 32 pass / 0 fail
Suíte completa do repositório:            572 tests, 553 pass, 8 fail (pré-existentes,
                                           idênticas às de sempre), 11 skip, 0 regressão nova
npm run build:                            sucesso; rota de dev e editor continuam
                                           fora do bundle de produção
git diff --check:                         limpo
eslint (src/ + testes editoriais):        120 problemas, todos pré-existentes, 0 novos
```

Nenhuma alteração de frontend, Edge Function ou lógica de negócio nesta
correção — só a migration e os testes. `deno check` das Edge Functions do
domínio novo não foi reexecutado por não haver nenhum arquivo `.ts` tocado
por esta correção (permanece limpo desde a auditoria original).
