...
async function runGeneratorModel({
  ...
  const calls = { legacyTemplateRotation: 0, sponsorRpc: 0, editorial: 0, profileLookup: 0 }
  ...
  const params = {
    ...
    p_render_snapshot_base: { master_config: { id: config.id, master_template_uuid: config.master_template_uuid, enabled: true }, layer_map: config.layer_map },
  }
...
test('terminal idempotent retries do not invoke editorial or any rotation again', async () => {
  const actual = await runGeneratorModel({ sponsorCount: 2, rpcResult: { reused: true, candidate_news: { ...CREATED, status: 'pending_render' } } })
...
test('a concurrent retry with an existing processing claim does not invoke a second editorial workflow', async () => {
  const actual = await runGeneratorModel({ sponsorCount: 1, rpcResult: { reused: true, candidate_news: CREATED }, claim: false })
...
test('a retry after an interrupted editorial stage may claim the same candidate without re-rotating', async () => {
  const actual = await runGeneratorModel({ sponsorCount: 1, rpcResult: { reused: true, candidate_news: CREATED }, claim: true })
