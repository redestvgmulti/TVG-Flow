...
test('implemented generator delegates sponsor rotation to the single database RPC', async () => {
  ...
  // The vehicle fixes the sponsor count; the RPC receives the derived value.
  assert.match(source, /p_sponsor_count: effectiveSponsorCount/)
  assert.match(source, /const effectiveSponsorCount = usesVehicle \? sponsorCountForVehicle\(veiculo\)/)
