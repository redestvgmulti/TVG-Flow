...
test('editorial UI separates production, review, approval and publication states', async () => {
  ...
  assert.match(ui, /\{ key: 'em_producao', label: 'Produzindo' \}/)
  assert.match(ui, /\{ key: 'revisao', label: 'Para revisar' \}/)
  assert.match(ui, /pending_render: 'em_producao'/)
  assert.match(ui, /pending_review: 'revisao'/)
  assert.match(ui, /if \(currentTab === 'aprovadas'\) return \['approved'\]/)
  assert.doesNotMatch(ui, /mark_candidate_news_posted/)
  assert.match(ui, /Publicar pelo sistema está temporariamente indisponível/)
