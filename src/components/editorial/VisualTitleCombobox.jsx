...
export default function VisualTitleCombobox({
  groups = [],
  value,
  onChange,
  contentType,
  ...
  onRetry,
  fieldError,
  label = 'Selo da matéria',
  emptyLabel = 'Selecione um selo',
  ...
  return <div style={{ position: 'relative', display: 'grid', gap: 8 }}>
    <label id={`${listId}-label`} style={{ fontSize: 14, fontWeight: 600, color: '#334155' }}>{label} <span style={{ color: '#ef4444' }}>*</span></label>
    <button
      ref={triggerRef}
      type="button"
      role="combobox"
    ...
    >
      {selected ? <><Preview title={selected} /><span style={{ flex: 1 }}>{selected.nome}</span></> : <span style={{ flex: 1, color: '#64748b' }}>{loading ? 'Carregando selos...' : emptyLabel}</span>}
    ...
    {open && <div id={listId} role="listbox" aria-labelledby={`${listId}-label`} style={{ position: 'absolute', zIndex: 20, top: '100%', left: 0, right: 0, maxHeight: 320, overflowY: 'auto', background: '#fff', border: '1px solid #cbd5e1', borderRadius: 12, boxShadow: '0 12px 28px rgba(15,23,42,.16)', padding: 8 }}>
      <div style={{ position: 'sticky', top: -8, padding: '4px 0 8px', background: '#fff', zIndex: 1 }}><label style={{ position: 'absolute', width: 1, height: 1, overflow: 'hidden' }} htmlFor={`${listId}-search`}>{searchPlaceholder}</label><div style={{ display: 'flex', alignItems: 'center', gap: 8, border: '1px solid #cbd5e1', borderRadius: 8, padding: '0 8px' }}><Search size={16} aria-hidden="true" /><input ref={inputRef} id={`${listId}-search`} value={query} onChange={event => setQuery(event.target.value)} onKeyDown={onSearchKeyDown} aria-activedescendant={activeOptionId} placeholder={searchPlaceholder} style={{ width: '100%', border: 0, outline: 0, padding: 10 }} /></div></div>
